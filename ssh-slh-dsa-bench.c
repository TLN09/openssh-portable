...
int benchmark_keygeneration(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Key Generation", bits);
...
int key_serialization_private(struct sshkey *k, struct sshbuf *buffer, enum sshkey_serialize_rep options) {
    ...
    r = ssh_slh_dsa_serialize_private(k, buffer, options);
...
int benchmark_serialization_private(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Serialization Private", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
...
int key_serialization_public(struct sshkey *k, struct sshbuf *buffer, enum sshkey_serialize_rep options) {
    ...
    r = ssh_slh_dsa_serialize_public(k, buffer, options);
...
int benchmark_serialization_public(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Serialization Public", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
        struct sshbuf *buffer = sshbuf_new();
        key_generation(key, bits);
...
int key_deserialization_private(struct sshkey *k, struct sshbuf *buffer) {
    ...
    r = ssh_slh_dsa_deserialize_private("ssh-ml-dsa", buffer, k);
...
int benchmark_deserialization_private(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Deserialization Private", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
        struct sshbuf *buffer = sshbuf_new();
        key_generation(key, bits);
...
int key_deserialization_public(struct sshkey *k, struct sshbuf *buffer) {
    ...
    r = ssh_slh_dsa_deserialize_public("ssh-ml-dsa", buffer, k);
...
int benchmark_deserialization_public(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Deserialization Public", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
        struct sshbuf *buffer = sshbuf_new();
        key_generation(key, bits);
...
int signing(struct sshkey *key,
    ...
    r = ssh_slh_dsa_sign(key, sigp, lenp, data, datalen, NULL, NULL, NULL, 0);
...
int benchmark_signing(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Signing", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
        ssh_slh_dsa_generate(key, bits);
        u_char *sig = NULL;
...
int verification(
    ...
    r = ssh_slh_dsa_verify(key, sig, siglen, data, datalen, NULL, 0, NULL);
...
int benchmark_verification(int bits, u64 *deltas) {
    ...
    char *type;
    asprintf(&type, "SLH-DSA-%d Verification", bits);
    for(j = 0, k = 0; j < ITERATIONS; j++) {
        struct sshkey *key = sshkey_new(KEY_SLH_DSA);
        ssh_slh_dsa_generate(key, bits);
        u_char *sig = NULL;
...
int main(int argc, char **argv) {
    ...
    benchmark_keygeneration(128, deltas);
    benchmark_keygeneration(129, deltas);
    benchmark_keygeneration(192, deltas);
    benchmark_keygeneration(193, deltas);
    benchmark_keygeneration(256, deltas);
    benchmark_keygeneration(257, deltas);
    
    benchmark_serialization_private(128, deltas);
    benchmark_serialization_private(129, deltas);
    benchmark_serialization_private(192, deltas);
    benchmark_serialization_private(193, deltas);
    benchmark_serialization_private(256, deltas);
    benchmark_serialization_private(257, deltas);
    
    benchmark_serialization_public(128, deltas);
    benchmark_serialization_public(129, deltas);
    benchmark_serialization_public(192, deltas);
    benchmark_serialization_public(193, deltas);
    benchmark_serialization_public(256, deltas);
    benchmark_serialization_public(257, deltas);
    
    benchmark_deserialization_private(128, deltas);
    benchmark_deserialization_private(129, deltas);
    benchmark_deserialization_private(192, deltas);
    benchmark_deserialization_private(193, deltas);
    benchmark_deserialization_private(256, deltas);
    benchmark_deserialization_private(257, deltas);
    
    benchmark_deserialization_public(128, deltas);
    benchmark_deserialization_public(129, deltas);
    benchmark_deserialization_public(192, deltas);
    benchmark_deserialization_public(193, deltas);
    benchmark_deserialization_public(256, deltas);
    benchmark_deserialization_public(257, deltas);

    benchmark_signing(128, deltas);
    benchmark_signing(129, deltas);
    benchmark_signing(192, deltas);
    benchmark_signing(193, deltas);
    benchmark_signing(256, deltas);
    benchmark_signing(257, deltas);

    benchmark_verification(128, deltas);
    benchmark_verification(129, deltas);
    benchmark_verification(192, deltas);
    benchmark_verification(193, deltas);
    benchmark_verification(256, deltas);
    benchmark_verification(257, deltas);
